...
class CodeTranslator {
  ...
  _detectLanguage(code) {
    if (/require\(|module\.exports|const\s+|let\s+|=>|console\.log/.test(code) && !/def\s+/.test(code)) return 'javascript';
  ...
  _buildMapping(from, to) {
    const pairs = {
      'javascript': {
        'python': {
          ...
          '=>': (line) => line.replace(/\(([^)]*)\)\s*=>\s*{?([^}]*)}?/, (_, p, b) => `lambda ${p}: ${b.trim()}`),
    ...
    return pairs[from]?.[to] || { notes: [`No direct mapping from ${from} to ${to}. Basic syntax only.`] };
  }
  ...
  _translateLine(line, mapping, from, to) {
    ...
    const transforms = ['const', 'let', 'function', 'def', '=>', 'console.log', 'print',
                        'if', 'else', 'elif', 'else:', 'null', 'None', 'true', 'True', 'false', 'False',
                        '.push', '.append', 'length', 'semicolon', '.append('];
