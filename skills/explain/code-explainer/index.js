...
class CodeExplainer {
  ...
  _extractFunctions(code) {
    ...
    for (const pat of patterns) {
      ...
      while ((m = pat.exec(code)) !== null) {
        ...
        if (!funcs.find(f => f.name === name)) {
          funcs.push({ name, params: m[2] ? m[2].split(',').map(p => p.trim()).filter(Boolean) : [] });
        }
      }
    }
    const lines = code.split('\n');
    return funcs.map(f => ({
      ...f,
      lineCount: lines.filter(l => l.includes(f.name) && (l.includes('function') || l.includes('=>') || l.includes('def ') || l.includes('fn '))).length
    }));
  }
  ...
  _estimateComplexity(code) {
    ...
    const branches = lines.filter(l =>
      /\bif\b|\belse\b|\belse if\b|\bfor\b|\bwhile\b|\bswitch\b|\bcase\b|\bcatch\b|\b&&\b|\b\|\|\b/.test(l)
    ).length;
  ...
  _generateSummary(struct, functions, classes, imports, complexity) {
    ...
    if (functions.length > 0) {
      ...
      parts.push(`It defines ${functions.length} function${functions.length > 1 ? 's' : ''}: ${names}.`);
    }

    if (classes.length > 0) {
      const names = classes.map(c => c.name + (c.extends ? ` (extends ${c.extends})` : '')).join(', ');
      parts.push(`It defines ${classes.length} class${classes.length > 1 ? 'es' : ''}: ${names}.`);
    }

    parts.push(`Complexity is ${complexity.difficulty} (${complexity.branches} conditional branches).`);

    if (complexity.difficulty === 'simple') parts.push('This code is straightforward and easy to follow.');
    else if (complexity.difficulty === 'moderate') parts.push('This code has some branching logic — review carefully.');
