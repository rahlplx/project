...
const COMMANDS = [
  { name: 'craft', description: 'Full shape-then-build flow with visual iteration' },
  {
    name: 'init',
    description:
      'One-time setup gathering design context (surface type, audience, voice, palette, typography)',
  },
  { name: 'document', description: 'Generate a DESIGN.md from existing code' },
  {
    name: 'extract',
    description: 'Pull reusable components and design tokens out of existing code',
  },
  ...
  {
    name: 'audit',
    description: 'Technical quality checks: accessibility, performance, anti-pattern detectors',
  },
...
const SUPPLEMENTAL_DETECTORS = {
  nestedCards: {
    ...
    name: 'Nested Cards',
    check: d =>
      Array.isArray(d.cards) && d.cards.some(c => Array.isArray(c.cards) && c.cards.length > 0),
  ...
  bounceEasing: {
    ...
    message:
      'Bounce/elastic easing on UI transitions reads as an AI-generated default, not an intentional choice.',
    severity: 'medium',
  ...
  grayTextOnColor: {
    ...
    check: d =>
      !!(
        ...
        d.color.textColor === 'gray'
      ),
    message:
      'Gray text loses contrast against a colored background and reads as an unstyled default.',
    severity: 'high',
  ...
  sideTabBorder: {
    ...
    message:
      'A thick colored left border on cards/tabs is a recognizable AI-generated layout tell.',
    severity: 'low',
  ...
  darkGlow: {
    ...
    message:
      'Large soft glow shadows around elements are an overused AI-generated dark-mode effect.',
    severity: 'low',
...
class ImpeccableAudit extends SkillBase {
  constructor() {
    ...
    this.version = '1.0.0';
    this.description =
      'Design quality workflow (init/shape/critique/audit/polish + 18 targeted refinement commands) with deterministic anti-pattern detectors, ported from the community impeccable skill.';
  ...
  getCommand(name) {
    ...
    if (!found) {
      return {
        type: 'error',
        message: `Unknown impeccable command: ${name}. Valid commands: ${COMMANDS.map(c => c.name).join(', ')}`,
      };
    }
    return found;
  }
  ...
  audit(design) {
    ...
    return {
      ...
      detectorCount:
        Object.keys(this.antiSlop.detectors).length + Object.keys(SUPPLEMENTAL_DETECTORS).length,
    };
  }
  ...
  critique(design = {}) {
    ...
    if (!design.hierarchy || design.hierarchy.levels < 2) {
      findings.push({
        axis: 'hierarchy',
        issue: 'No clear visual hierarchy between primary and secondary content.',
      });
    }
    if (!design.voice || !design.voice.matchesBrand) {
      findings.push({
        axis: 'resonance',
        issue: 'Copy/visual tone does not clearly match the stated brand voice.',
      });
    }
    ...
    if (design.genericness && design.genericness > 7) {
      findings.push({
        axis: 'resonance',
        issue: 'Design reads as generic/template-like rather than distinctive.',
      });
    }
    ...
    return {
      ...
      summary:
        findings.length === 0
          ? 'Design has clear hierarchy and resonates with the stated voice.'
          : `${findings.length} critique finding(s).`,
    };
  }
  ...
  polish(checklist = {}) {
    const items = [
      {
        id: 'contrast',
        label: 'All text meets WCAG AA contrast (4.5:1)',
        passed: !!checklist.contrast,
      },
      {
        id: 'responsive',
        label: 'Verified at mobile, tablet, and desktop breakpoints',
        passed: !!checklist.responsive,
      },
      {
        id: 'states',
        label: 'Hover, focus, active, and disabled states defined for interactive elements',
        passed: !!checklist.states,
      },
      {
        id: 'emptyStates',
        label: 'Empty/loading/error states designed, not just the happy path',
        passed: !!checklist.emptyStates,
      },
      {
        id: 'reducedMotion',
        label: 'Animations respect prefers-reduced-motion',
        passed: !!checklist.reducedMotion,
      },
    ];
