...
class TypographyRules {
  ...
  initializeLineHeightGuidelines() {
    return {
      ...
      loose: {
        min: 1.625,
        max: 2.0,
        use: ['Accessibility text', 'Large print', 'Learning materials'],
      },
    };
  }
  ...
  generateTypography(config = {}) {
    ...
    return {
      fontFamily: {
        heading: this.generateFontStack(
          fontFamily.heading.match(/"([^"]+)"/)?.[1] || 'Plus Jakarta Sans',
          'sans'
        ),
        body: this.generateFontStack(fontFamily.body.match(/"([^"]+)"/)?.[1] || 'Inter', 'sans'),
        display: this.generateFontStack(
          fontFamily.heading.match(/"([^"]+)"/)?.[1] || 'Plus Jakarta Sans',
          'sans'
        ),
