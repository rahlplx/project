...
class FlowchartGen {
  ...
  generateFlowchart(codeStructure, options = {}) {
    const structure = typeof codeStructure === 'string' 
      ? this.parseCode(codeStructure) 
      : codeStructure;
  ...
  extractEdges(structure) {
    ...
    for (const dep of structure.dependencies) {
      ...
      if (structure.imports.some(i => i.includes(dep))) {
        const sourceNode = structure.classes.length > 0 
    ...
    // Connect to exit
    if (structure.functions.length > 0) {
      edges.push({ from: `func_${structure.functions[structure.functions.length - 1].name}`, to: 'exit', label: '' });
  ...
  mightCall(cls, func, structure) {
    ...
    return funcLower.includes(classLower) || 
  ...
  toAscii(nodes, edges, options = {}) {
    ...
    while (queue.length > 0) {
      ...
      // Draw node
      const label = node.label.length > maxWidth - 10 
