...
const SECURITY_RULES = [
  ...
  },
  {
    ...
    checks: [
      ...
      'User input is sanitized before HTML output',
  ...
  {
    id: 'A05_security_misconfig',
    owasp: 'A05',
    name: 'Security Misconfiguration',
    severity: 'high',
    checks: [
      'No default passwords',
      'Error messages hide stack traces in prod',
  ...
  },
  {
    ...
    checks: [
      ...
      'Multi-factor auth available for sensitive actions',
    ],
  },
  {
    id: 'A08_software_integrity',
    owasp: 'A08',
  ...
  },
  {
    ...
    checks: [
      ...
      'Internal services not accessible via user URLs',
...
const CODE_PATTERNS = [
  {
    id: 'hardcoded_secret',
    pattern: /(?:api[_-]?key|secret|password|token)\s*[=:]\s*['"`][a-zA-Z0-9_-]{10,}/i,
    severity: 'critical',
    message: 'Possible hardcoded secret',
  },
  {
    id: 'sql_concat',
    pattern: /['"`]\s*\+\s*(?:req\.|params\.|body\.|query\.)/i,
    severity: 'critical',
    message: 'Possible SQL injection — string concatenation with user input',
  },
  {
    id: 'inner_html_var',
    pattern: /innerHTML\s*=\s*(?!['"`])[^;]+/,
    severity: 'high',
    message: 'innerHTML with variable — possible XSS',
  },
  {
    id: 'no_auth_check',
    pattern:
      /router\.(get|post|put|delete)\s*\(['"`][^'"` ]+['"`]\s*,\s*(?!auth|verify|require)\w+\s*\)/,
    severity: 'warning',
    message: 'Route handler without apparent auth middleware',
  },
  {
    id: 'cors_star',
    pattern: /cors\(\s*\{\s*origin\s*:\s*['"]\*['"]/,
    severity: 'high',
    message: 'CORS allows all origins — too permissive for production',
  },
  {
    id: 'md5_password',
    pattern: /md5\s*\(.*password|password.*md5/i,
    severity: 'critical',
    message: 'MD5 is not safe for password hashing — use bcrypt or argon2',
  },
];
