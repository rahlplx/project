...
class AntiPatterns {
  ...
  _getPatterns() {
    return [
      ...
      { id: 'AP05', category: 'style', severity: 'info', name: 'Long Lines', message: 'Lines longer than 100 characters are hard to read', suggestion: 'Break long lines at logical points', check: (c) => c.split('\n').some(l => l.length > 100) },
      { id: 'AP06', category: 'error', severity: 'critical', name: 'Empty Catch Blocks', message: 'Catch blocks that silently swallow errors', suggestion: 'Log the error or handle it appropriately', check: (c) => /catch\s*\([^)]*\)\s*\{\s*\}/g.test(c) },
      { id: 'AP07', category: 'error', severity: 'warning', name: 'Comparing Floats Directly', message: 'Floating point comparison may fail', suggestion: 'Use tolerance: Math.abs(a - b) < 0.0001', check: (c) => /\d+\.\d+\s*===\s*\d+\.\d+/.test(c) },
      { id: 'AP08', category: 'design', severity: 'info', name: 'God Object', message: 'Classes or objects with too many responsibilities', suggestion: 'Split into focused smaller classes', check: (c) => (c.match(/class\s+\w+/g) || []).length === 1 && c.length > 2000 },
      { id: 'AP09', category: 'code', severity: 'warning', name: 'Duplicate Code', message: 'Similar code blocks detected', suggestion: 'Extract repeated logic into a shared function', check: (c) => {
        const lines = c.split('\n');
        const unique = new Set(lines.map(l => l.trim()));
        return lines.length > 20 && unique.size < lines.length * 0.6;
      }},
      { id: 'AP10', category: 'style', severity: 'info', name: 'Missing Semicolons', message: 'Inconsistent semicolon usage in JS/TS', suggestion: 'Use a linter to enforce consistent style', check: (c) => {
        const jsLines = c.split('\n').filter(l => /^(const|let|var|return|console)/.test(l.trim()));
  ...
  toJSON() {
    return { name: this.name, version: this.version, description: this.description, patterns: this._getPatterns().length };
  }
}
