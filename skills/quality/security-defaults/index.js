...
class SecurityDefaults {
  ...
  _runChecks(code) {
    const checks = [
      ...
      { id: 'SEC05', name: 'HTTPS for external requests', category: 'network', passed: !/http:\/\/(?!localhost)/.test(code) || options.allowHttp },
      { id: 'SEC06', name: 'SQL injection protection', category: 'database', passed: !/SELECT.*\+|INSERT.*\+|query\(['"][^'"]*\$\{/.test(code) || !/sql|query|database/i.test(code) || options.skipDbCheck },
      { id: 'SEC07', name: 'No dangerous innerHTML', category: 'xss', passed: !/innerHTML\s*=/.test(code) || /DOMPurify|sanitize/.test(code) },
      { id: 'SEC08', name: 'Password hashing (if auth)', category: 'auth', passed: !/(password|passwd|pwd)/i.test(code) || /bcrypt|argon|hash|scrypt|pbkdf2/i.test(code) || options.skipAuthCheck },
      { id: 'SEC09', name: 'Rate limiting (if API)', category: 'api', passed: !/(app\.(get|post|put|delete|patch)|router\.)/i.test(code) || /rate.?limit|throttle|express-rate-limit/i.test(code) || options.skipRateLimit },
      { id: 'SEC10', name: 'CORS configured (if API)', category: 'api', passed: !/(app\.(get|post|put|delete|patch)|router\.)/i.test(code) || /cors|Access-Control/i.test(code) || options.skipCorsCheck }
    ];
    return checks;
  }
  ...
  toJSON() {
    return { name: this.name, version: this.version, description: this.description, reference: 'OWASP Top 10 (2021)' };
  }
}
