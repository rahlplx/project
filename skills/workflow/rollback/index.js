...
class Rollback {
  ...
  suggestRecovery(scenario) {
    ...
    if (/wrong (commit|branch)/i.test(lower)) return { type: 'undo-commit', command: this.buildCommand('undo-commit', 'HEAD'), explanation: 'Creates a new commit that undoes the bad one — safe for teams.' };
