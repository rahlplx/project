...
class Rollback extends SkillBase {
  ...
  buildCommand(type, ref) {
    const commands = {
      'undo-commit': {
        cmd: `git revert ${ref || 'HEAD'}`,
        desc: 'Creates a new commit that undoes the specified commit (safe for shared repos)',
        safe: true,
      },
      'undo-last-commit': {
        cmd: 'git reset --soft HEAD~1',
        desc: 'Undoes the last commit but keeps changes staged',
        safe: true,
      },
      'undo-all-local': {
        cmd: 'git checkout -- .',
        desc: 'Discards all unstaged local changes',
        safe: false,
        warning: 'This permanently discards uncommitted changes.',
      },
      'reset-to-commit': {
        cmd: `git reset --hard ${ref || 'HEAD~1'}`,
        desc: 'Resets to a specific commit, discarding all changes since',
        safe: false,
        warning: 'This permanently deletes all changes after the target commit.',
      },
      'revert-file': {
        cmd: `git checkout -- ${ref || 'file.js'}`,
        desc: 'Revert a specific file to its last committed state',
        safe: true,
      },
      'stash-pop': {
        cmd: 'git stash pop',
        desc: 'Restore the most recently stashed changes',
        safe: true,
      },
      'undo-merge': {
        cmd: `git revert ${ref || 'HEAD'} -m 1`,
        desc: 'Undo a merge commit',
        safe: true,
      },
    };
  ...
  suggestRecovery(scenario) {
    ...
    return {
      ...
      explanation: 'Safely undo your last commit.',
    };
  }
