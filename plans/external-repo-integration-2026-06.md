...
|------|---------|-----------|
| `profullstack/vibe-stack` | **Skip** | Name collision only. It's a PWA app boilerplate (Supabase backend, Android/iOS/desktop/WebXR builds, Puppeteer PDF gen) — an end-user app framework, not an agent-orchestration/skill system. Nothing in it overlaps with what this repo does. |
...
- Any literal file/directory copying from any of the six repos — every port above is a
  re-implementation of a *concept*, written against this repo's existing module shape and
