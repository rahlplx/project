...
1. Add entry to `catalog/tools.yaml` with required fields: `id`, `name`, `category`, `description`, `what_it_does`, `how_agent_uses`, `install`, `verify`
2. Verify: ≥100 stars, last commit < 6 months, no critical CVEs, OSS license (MIT/Apache-2.0/BSD-3-Clause)
3. Record in `catalog/verified-by.md` with your handle and date
4. Run `npm run lint` — YAML validity is a pre-commit gate
