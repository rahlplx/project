...
## Symptom
`package.json` contains dependencies that are never imported in source code. In vibenexus: `chalk ^5.3.0` and `ora ^7.0.1` were declared but zero callers existed across 50 skills and all lib/bin files.
...
## Incident
vibenexus, 2026-06-14: chalk and ora removed after audit found zero `require('chalk')` or `require('ora')` calls. 15 packages pruned from node_modules.
