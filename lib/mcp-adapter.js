...
class MCPAdapter {
  ...
  #extractParamOrder(instance, methodName) {
    ...
    for (const [key, { instance }] of Object.entries(skills)) {
      ...
      for (const method of methodNames) {
        if (
          ...
          const paramOrder = this.#extractParamOrder(instance, method);
  ...
  async callToolsBatch(toolCalls) {
    const tasks = toolCalls.map(({ name, arguments: args }) => {
      ...
      const safeArgs = this.#sanitizeArgs(args);
      const ordered =
        entry.paramOrder.length > 0
          ? entry.paramOrder.map(p => safeArgs[p])
          : Object.values(safeArgs);
      return {
