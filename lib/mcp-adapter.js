...
class MCPAdapter {
  ...
  #registerHandlers() {
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request) => {
      ...
      if (!entry) {
        return TokenOptimizer.wrapError(
      ...
      try {
        const fn = entry.instance[entry.method];
  ...
  async callToolsBatch(toolCalls) {
    const tasks = toolCalls.map(({ name, arguments: args }) => {
      const entry = this.toolMap[name];
      if (!entry) {
        return Promise.resolve(TokenOptimizer.wrapError(new Error(`Unknown tool: ${name}`), -32602));
      }
      return {
    ...
    return results.map((result, index) => {
      ...
      return {
        content: [{ type: 'text', text: wrapped.text }],
        isError: false,
        _meta: wrapped.truncated ? { truncated: true, originalTokens: wrapped.originalTokens } : undefined,
      };
    });
  }
