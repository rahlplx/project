...
class MCPAdapter {
  ...
  validateShellCommand(command) {
    ...
    if (forbidden.test(command)) {
      throw new Error('FORBIDDEN_COMMAND: Direct exit commands are not allowed in the sandbox. Use assertion outputs for test validation.');
    }
  }
    ...
    for (const [key, { instance }] of Object.entries(skills)) {
      ...
      for (const method of methodNames) {
        if (
          ...
          map[toolName] = { instance, method, key };
        }
      }
    }
    return map;
  ...
  #registerHandlers() {
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async request => {
      ...
      try {
        ...
        // Intercept and validate shell commands if this looks like an exec/spawn call
        if (name.includes('exec') || name.includes('spawn') || name.includes('run_command')) {
          const cmd = safeArgs.command || safeArgs.cmd || Object.values(safeArgs)[0];
          if (typeof cmd === 'string') {
            this.validateShellCommand(cmd);
          }
        }

        const result = await Promise.resolve(
          entry.instance[entry.method](...Object.values(safeArgs))
        );
  ...
  async callToolsBatch(toolCalls) {
    const tasks = toolCalls.map(({ name, arguments: args }) => {
      ...
      return {
        ...
        method: entry.method,
        args: Object.values(this.#sanitizeArgs(args)),
      };
    });
