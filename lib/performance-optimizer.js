...
class BatchProcessor {
  ...
  async run(tasks) {
    ...
    for (let i = 0; i < tasks.length; i += this.maxConcurrency) {
      ...
      const settled = await Promise.allSettled(batch.map(task => this.#executeSingle(task)));
  ...
  async #executeSingle(task) {
    ...
    throw new Error('BatchProcessor: task must be a function or {instance, method, args}');
  }
}
