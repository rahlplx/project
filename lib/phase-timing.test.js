...
describe('phase-timing', () => {
  it('creates file on first call', withIsolatedDir((dir) => {
  ...
  it('appends records', withIsolatedDir((dir) => {
    const start = new Date('2026-06-14T10:00:00Z');
    const end = new Date('2026-06-14T10:00:30Z');
    trackPhaseTiming('build', start, end);
  ...
  it('computes durationMs', withIsolatedDir((dir) => {
  ...
  it('records ISO string timestamps', withIsolatedDir((dir) => {
  ...
  it('getPhaseStats returns correct shape', withIsolatedDir((dir) => {
    trackPhaseTiming('build', new Date('2026-06-14T10:00:00Z'), new Date('2026-06-14T10:00:30Z'));
  ...
  it('getPhaseStats computes min/max/avg/median', withIsolatedDir((dir) => {
    trackPhaseTiming('stats-test', new Date('2026-06-14T13:00:00Z'), new Date('2026-06-14T13:00:10Z'));
    trackPhaseTiming('stats-test', new Date('2026-06-14T13:01:00Z'), new Date('2026-06-14T13:01:20Z'));
    trackPhaseTiming('stats-test', new Date('2026-06-14T13:02:00Z'), new Date('2026-06-14T13:02:30Z'));
  ...
  it('getPhaseStats returns zeroed stats for unknown phase', withIsolatedDir((dir) => {
  ...
  it('getAllTimingSummary includes all phases combined', withIsolatedDir((dir) => {
    trackPhaseTiming('build', new Date('2026-06-14T10:00:00Z'), new Date('2026-06-14T10:00:30Z'));
  ...
  it('handles empty file gracefully', withIsolatedDir((dir) => {
    const stats = getPhaseStats('any');
    assert.strictEqual(stats.count, 0);
  }));

  it('handles corrupt JSON by backing up and starting fresh', withIsolatedDir((dir) => {
  ...
  it('invalid timestamp throws TIMING_PARSE_ERROR', withIsolatedDir((dir) => {
    assert.throws(() => {
