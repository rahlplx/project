...
describe('parseVerifiedBy', () => {
  ...
  test('tier2 community-vibenexus', () => {
    expect(parseVerifiedBy('community-vibenexus')).toEqual({ tier: 2, starCount: null });
  });
...
describe('trustworthiness scoring', () => {
  ...
  test('tier2 community-vibenexus scores 12', () => {
    const tool = makeTool({ verified_by: 'community-vibenexus' });
    const result = computeToolScore(tool, makeOpts());
