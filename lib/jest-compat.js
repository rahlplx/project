...
function buildExpect(actual) {
  const notChain = {
    ...
    toEqual: expected => {
      ...
      if (!threw) assert.fail(`Expected values NOT to be deeply equal:\n  ${JSON.stringify(actual)}`);
    },
  ...
  return {
    ...
    toMatchObject: expected => {
      for (const [k, v] of Object.entries(expected)) {
        assert.deepStrictEqual(actual[k], v, `Key "${k}": expected ${JSON.stringify(v)}, got ${JSON.stringify(actual[k])}`);
      }
    },
    ...
    toHaveBeenCalledWith: (...expected) => {
      ...
      const matched = calls.some(
        call => call.length === expected.length && expected.every((e, i) => {
          try { assert.deepStrictEqual(call[i], e); return true; } catch { return false; }
        })
      );
...
const testFn = nodeTest.test;
testFn.each = items => (name, fn) => {
  for (const item of (Array.isArray(items) ? items : [items])) {
    const label = typeof name === 'string'
      ? name.replace('%s', String(item)).replace('%p', JSON.stringify(item)).replace('%i', String(item))
      : String(name);
    nodeTest.test(label, async t => fn(item, t));
  }
};

// describe.each shim
const describeFn = nodeTest.describe;
describeFn.each = items => (name, fn) => {
  for (const item of (Array.isArray(items) ? items : [items])) {
    const label = typeof name === 'string'
      ? name.replace('%s', String(item)).replace('%p', JSON.stringify(item))
      : String(name);
    nodeTest.describe(label, () => fn(item));
  }
};
