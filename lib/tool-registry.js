class ToolRegistry {
  ...
  async findUsable(category) {
    const toolsInCategory = Array.from(this.tools.values()).filter(t => t.category === category);
    ...
    for (const tool of toolsInCategory) {
      try {
        ...
        if (!usable) {
          results.push({
            name: tool.name,
            reason: 'isUsable returned false',
        ...
        results.push({
          name: tool.name,
          reason: e.message || 'isUsable threw',
          metadata: tool.metadata,
        });
      }
    }
  ...
  async _runWithTimeout(fn, timeout = 3000) {
    return Promise.race([
      Promise.resolve(fn()),
      new Promise((_, reject) => setTimeout(() => reject(new Error('isUsable timeout')), timeout)),
    ]);
  }
}
