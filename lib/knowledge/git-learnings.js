...
function extract(cwd = process.cwd(), options = {}) {
  ...
  const exec = (cmd) => execSync(cmd, { cwd, encoding: 'utf8', timeout: 10000 }).trim();

  let recentCommits = [];
  try {
    const log = exec(`git log --oneline -${limit}`);
    recentCommits = log.split('\n').filter(Boolean).map((line) => {
      const [sha, ...rest] = line.split(' ');
      return { sha, message: rest.join(' ') };
    });
  } catch {
    return { error: 'Not a git repository or git unavailable.', recentCommits: [], topChangedFiles: [], commitTypes: {}, velocity: null };
  }
  ...
  let topChangedFiles = [];
  try {
    const shortstat = exec(`git log --name-only --pretty=format: -${Math.min(limit, 30)}`);
    const fileCounts = {};
  ...
  try {
    ...
    const count = exec(`git log --oneline --since="${since}" | wc -l`);
    velocity = { commitsLast7Days: parseInt(count, 10) || 0 };
  } catch { /* skip */ }

  const antiPatterns = [];
  if ((commitTypes.fix || 0) > (commitTypes.feat || 0)) {
    antiPatterns.push('More fix commits than feat commits — high defect rate, consider adding tests before new features.');
  }
  ...
  return { recentCommits: recentCommits.slice(0, 10), topChangedFiles, commitTypes, velocity, antiPatterns };
}
