...
function checkIndexIntegrity(options = {}) {
  ...
  const missing = current.skills.filter(s => !existingMap.has(s.name)).map(s => s.name);

  const extra = existing.skills.filter(s => !currentMap.has(s.name)).map(s => s.name);
  ...
  if (missing.length > 0) {
    errors.push({
      type: 'missing-skills',
      detail: `${missing.length} skills missing from index: ${missing.join(', ')}`,
    });
  }
  if (extra.length > 0) {
    errors.push({
      type: 'extra-skills',
      detail: `${extra.length} stale entries in index: ${extra.join(', ')}`,
    });
  }
