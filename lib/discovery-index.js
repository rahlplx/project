...
function buildIndex(options = {}) {
  ...
  for (const file of files) {
    try {
      ...
      // Extract description from SKILL.md — try frontmatter first, then first paragraph after heading
      let description = '';
      ...
      try {
        if (fs.existsSync(skillMd)) {
          ...
          if (!description) {
            // Fallback: first non-blank paragraph after the # title
            ...
            if (paragraphLines.length > 0) description = paragraphLines.join(' ');
          }
        }
      } catch {
        /* degrade */
