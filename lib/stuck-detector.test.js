...
describe('stuck-detector', () => {
  ...
  it('checkStuck reads from file when no history given', withIsolatedDir((dir) => {
    trackPhase('review');
    trackPhase('review');
    trackPhase('review');
    trackPhase('review');
  ...
  it('getStuckAlerts returns empty when none stuck', withIsolatedDir((dir) => {
  ...
  it('getStuckAlerts detects multiple stuck phases', withIsolatedDir((dir) => {
