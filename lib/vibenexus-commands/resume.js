...
const handler = (args, state) => {
  ...
  if (!state) {
    ...
    console.log('  Run: npx vibenexus help');
  ...
  if (nextPhase) {
    console.log(`\n  \x1b[1mSuggested next:\x1b[0m npx vibenexus ${nextPhase}`);
  } else if (info.phase === 'done') {
    console.log('\n  \x1b[1mProject complete.\x1b[0m Run npx vibenexus auto to restart pipeline.');
  }
