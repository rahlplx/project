...
function renderStatus() {
  ...
  const resolvedBlockers = (latestSession && latestSession.blockers)
    ? latestSession.blockers.filter(b => b.resolution).length : 0;

  // Build phase efficiency line
  const phaseNames = Object.keys(phaseStats).sort();
  const phaseLine = phaseNames.map(p => {
  ...
  function padMid(text) {
    const esc = (function() { return '\x1b'; })();
    const clean = text.replace(new RegExp(esc + '\\[[0-9;]*m', 'g'), '').replace(/[^\x20-\x7E\u2500-\u257F]/g, '');
  ...
  lines.push('┌' + '─'.repeat(w) + '┐');
  lines.push(padMid('📊 vibe-stack Diagnostics'));
  lines.push('├' + '─'.repeat(w) + '┤');
  ...
  lines.push(padMid(`Errors:             ${errorStats.total} (${resolvedErrors} resolved, ${activeErrors} active)`));
  lines.push(padMid(`Blockers:           ${blockerCount} (${resolvedBlockers} resolved)`));
  lines.push(padMid(`Longest Phase:      ${longestPhaseStr}`));

  if (harnessSummary) {
    lines.push(padMid(`Harness Checks:     ${harnessSummary.checksPassed}/${harnessSummary.checksRun} passed`));
  }
  ...
  if (latestSession && latestSession.compaction && latestSession.compaction.compaction_warning_signals) {
    lines.push('├' + '─'.repeat(w) + '┤');
