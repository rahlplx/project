...
describe('telemetry-session-schema', () => {
  ...
  it('latest session has user_interactions with dynamic commands', () => {
    ...
    assert.strictEqual(typeof session.user_interactions.times_corrected_ai, 'number', 'times_corrected_ai is number');
    assert.strictEqual(typeof session.user_interactions.times_asked_clarification, 'number', 'times_asked_clarification is number');
  });
  ...
  it('latest session phases have reviews_passed and reviews_failed', () => {
    ...
    for (const name of phaseNames) {
      ...
      assert.strictEqual(typeof p.reviews_passed, 'number', `phase '${name}' reviews_passed is number`);
      assert.strictEqual(typeof p.reviews_failed, 'number', `phase '${name}' reviews_failed is number`);
      assert.strictEqual(typeof p.tasks_completed, 'number', `phase '${name}' tasks_completed is number`);
