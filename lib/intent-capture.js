...
function sanitizeText(text, maxLength = 2000) {
  ...
  return (
    text
      ...
      .replace(/[\u202A-\u202E\u2066-\u2069\u200B-\u200D\u2028-\u2029\uFEFF]/g, '') // Strip Unicode bidi/zero-width injectors
      .trim()
      .slice(0, maxLength)
  );
}
...
function validateInput(input) {
  ...
  textFields.forEach(field => {
    if (safeInput[field] && typeof safeInput[field] === 'string' && safeInput[field].length > 2000) {
      errors.push(
...
function captureIntent(input) {
  const safeInput = input || {};
  const projectName = sanitizeProjectName(safeInput.projectName || 'Untitled Project');
  ...
  const prdData = {
    projectName,
    overview: safeInput.solution || 'Not specified',
