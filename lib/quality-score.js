...
function parseVerifiedBy(verifiedBy) {
  ...
  if (v === 'community-vibenexus') return { tier: 2, starCount: null };
...
function parseDateFreshness(verifiedDate, referenceDate) {
  ...
  const diffMs = ref.getTime() - verified.getTime();
...
function computeToolScore(toolEntry, options = {}) {
  ...
  const total =
    maintenance.score + docs.score + community.score + trustworthiness.score + freshness.score;
  return {
    name: toolEntry.name,
      ...
      score_dimensions: [
        {
          name: 'maintenance',
          max: 30,
          description: 'License, skills inclusion, description quality, skill path',
        },
...
function formatQualityReport(results, _options = {}) {
  const lines = [];
  ...
  if (results.summary) {
    ...
    lines.push(
      `    Distribution: A=${results.summary.distribution.A}  B=${results.summary.distribution.B}  C=${results.summary.distribution.C}  D=${results.summary.distribution.D}`
    );
    lines.push('');
  }
  if (results.tools.length > 0) {
    const header =
      '  Tool'.padEnd(32) + 'Score'.padEnd(8) + 'Grade'.padEnd(6) + 'Maint Doc Comm Trust Fresh';
    lines.push(header);
