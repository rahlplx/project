...
describe('null/undefined input guards', () => {
  // Regression tests — captureIntent and validateInput previously threw TypeError
  // on null/undefined input (call path: lib/legacy-orchestrator.js → lib/mcp/tools.js).
  it('captureIntent(null) does not throw and returns defaults', () => {
    assert.doesNotThrow(() => captureIntent(null));
    const result = captureIntent(null);
    assert.ok(result.projectMd.includes('# Untitled Project'));
    assert.ok(result.prdMd);
    assert.ok(result.projectType);
  });

  it('captureIntent(undefined) does not throw and returns defaults', () => {
    assert.doesNotThrow(() => captureIntent(undefined));
    const result = captureIntent(undefined);
    assert.ok(result.projectMd.includes('# Untitled Project'));
  });

  it('validateInput(null) does not throw and returns invalid', () => {
    assert.doesNotThrow(() => validateInput(null));
    const result = validateInput(null);
    assert.strictEqual(result.valid, false);
    assert.ok(result.errors.length > 0);
    assert.ok(result.errors.includes('Project name is required'));
  });

  it('validateInput(undefined) does not throw and returns invalid', () => {
    assert.doesNotThrow(() => validateInput(undefined));
