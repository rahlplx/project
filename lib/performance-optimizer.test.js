...
const { TTLCache, ResponseStreamer, BatchProcessor, SkillCache, wrapToolCall } = require('./performance-optimizer');
...
describe('BatchProcessor', () => {
  ...
  it('runs functions in batches', async () => {
    ...
    assert.deepStrictEqual(results.map(r => r.value), [1, 2, 3, 4]);
  });
  ...
  it('respects maxConcurrency', async () => {
    ...
    const tasks = [
      ...
      () => { order.push(2); return Promise.resolve(2); },
      () => { order.push(3); return Promise.resolve(3); },
      () => { order.push(4); return Promise.resolve(4); },
    ];
