...
const SAFE_ATTR_KEYS = new Set([
  'tracer', 'phase', 'command', 'status', 'duration', 'skills', 'confidence',
  'label', 'source', 'requestId', 'parentSpanId', 'model', 'template',
  'template.category', 'template.id', 'error.message', 'error.type',
  'enricher.sources', 'enricher.confidence', 'harness.checks', 'harness.passed',
  'lifecycle.interaction_count', 'lifecycle.maintenance_count',
]);
...
function redactAttributes(attrs) {
  ...
  for (const [k, v] of Object.entries(attrs)) {
    if (SAFE_ATTR_KEYS.has(k)) {
      ...
      safe[k] = typeof v === 'string' && v.length <= 200 ? v
