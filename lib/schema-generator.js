...
class SchemaGenerator {
  ...
  static #getCacheKey(fn, jsdoc) {
    ...
    const hash = crypto.createHash('md5').update(fnStr + jsdoc).digest('hex').slice(0, 16);
  ...
  static #generateSchema(fn, jsdoc) {
    ...
    for (const param of params) {
      const jsdocInfo = jsdocParams[param.name] || {};
      const type = this.#inferType(param, jsdocInfo);

      properties[param.name] = {
        type,
        description: jsdocInfo.description || this.#defaultDescription(param.name),
      };
  ...
  static #parseParam(paramStr, index) {
    ...
    if (trimmed.startsWith('...')) {
      ...
      if (leftTrimmed.startsWith('{') && leftTrimmed.endsWith('}')) {
        destructured = leftTrimmed.slice(1, -1).split(',').map(s => s.trim()).filter(Boolean);
        name = 'options';
      } else {
        name = leftTrimmed;
      }
      ...
      destructured = trimmed.slice(1, -1).split(',').map(s => s.trim()).filter(Boolean);
      name = 'options';
    } else {
      name = trimmed;
    }
  ...
  static #parseJSDoc(jsdoc) {
    ...
    const cacheKey = crypto.createHash('md5').update(jsdoc).digest('hex').slice(0, 16);
  ...
  static #inferFromName(name) {
    ...
    if (lower.includes('path') || lower.includes('dir') || lower.includes('file') || lower.includes('url') || lower.includes('name') || lower.includes('id') || lower.includes('key')) return 'string';
    if (lower.includes('count') || lower.includes('num') || lower.includes('size') || lower.includes('limit') || lower.includes('max') || lower.includes('min') || lower.includes('port') || lower.includes('timeout')) return 'number';
    if (lower.startsWith('is') || lower.startsWith('has') || lower.startsWith('can') || lower.startsWith('should') || lower.startsWith('enable') || lower.startsWith('disable') || lower.includes('flag')) return 'boolean';
    if (lower.includes('list') || lower.includes('items') || lower.includes('array') || lower.endsWith('s')) return 'array';
    if (lower.includes('options') || lower.includes('config') || lower.includes('settings') || lower.includes('params')) return 'object';
    return 'string';
  }
