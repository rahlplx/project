  ...
  scheduleProcessing() {
    ...
    const process = () => {
      ...
      if (this.queue.length > 0) {
        const wait = Math.ceil((1 - this.tokens) / this.fillRate);
...
class SubagentDispatch {
  constructor() {
    ...
    this.limiter = new TokenBucket(10, 10 / (60 * 1000));
  }

  setRateLimit(requests, intervalMs) {
    this.limiter = new TokenBucket(requests, requests / intervalMs);
  }
  ...
  async dispatch(subagent, prompt, options = {}) {
    ...
    // Wait for rate limit ticket
    await this.limiter.getToken();

    const fullPrompt = this.buildNonInteractivePrompt(sanitizeText(prompt, 8000), agent);
...
`;
  ...
  /**
   * Returns capability flags for this dispatch implementation.
   * experimentalExecution: true means executeWithTimeout is a stub — no real agent process is spawned.
   */
  getCapabilities() {
    return { experimentalExecution: true, platform: 'stub' };
  }

  /**
   * Execute with timeout
   * @experimental — currently a stub; real agent spawning not yet implemented.
   */
