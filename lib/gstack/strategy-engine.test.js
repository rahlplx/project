...
describe('StrategyEngine', () => {
  ...
  it('should score narrow wedge criteria against the thinkDoc', () => {
    const wedge = strategyEngine.identifyNarrowWedge({ mvp: 'x', problem: 'y', successMetrics: 'DAU +10%' });
  ...
  it('should perform engineer review', () => {
    const designDoc = { failureModes: [{ name: 'DB timeout', mitigation: 'retry with backoff' }], testCoverage: 85 };
  ...
  it('should build a review report with REQUEST_CHANGES verdict on a high-severity finding', () => {
    const report = strategyEngine.buildReviewReport([{ severity: 'high', message: 'Missing auth check' }]);
  ...
  it('should build a review report with APPROVE_WITH_CONCERNS verdict on medium-only findings', () => {
    const report = strategyEngine.buildReviewReport([{ severity: 'medium', message: 'Minor contrast issue' }]);
