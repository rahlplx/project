...
class StrategyEngine {
  ...
  checkPrimeDirectives(thinkDoc) {
    const checks = [
      { directive: PRIME_DIRECTIVES[0], met: Array.isArray(thinkDoc.failureModes) && thinkDoc.failureModes.length > 0 },
      { directive: PRIME_DIRECTIVES[2], met: !!thinkDoc.shadowPaths },
      { directive: PRIME_DIRECTIVES[4], met: !!thinkDoc.observability },
      { directive: PRIME_DIRECTIVES[6], met: Array.isArray(thinkDoc.outOfScope) && thinkDoc.outOfScope.length > 0 }
    ];
  ...
  detectAISlop(designDoc) {
    ...
    return {
      ...
      severity: result.violations.length > 0 ? 'high' : (result.warnings.length > 0 ? 'medium' : 'none')
    };
  }
  ...
  generateDesignerRecommendations(designDoc) {
    const slop = this.detectAISlop(designDoc);
    if (slop.detected) {
      return ['Resolve anti-slop findings before shipping.', 'Test with real users.', 'Iterate based on feedback.'];
    }
    return ['Design passes anti-slop checks.', 'Test with real users.', 'Iterate based on feedback.'];
  }
  ...
  identifyFailureModes(designDoc) {
    if (Array.isArray(designDoc.failureModes) && designDoc.failureModes.length) {
      return designDoc.failureModes.map((mode) => ({
        mode: mode.name || mode,
        named: !!mode.name,
        mitigation: mode.mitigation || 'Not specified — violates Prime Directive 2 (every error has a name).'
      }));
    }
    return [{ mode: 'UNNAMED', named: false, mitigation: 'No failure modes declared — violates Prime Directive 1 (zero silent failures).' }];
  }
  ...
  generateEngineerRecommendations(designDoc) {
    ...
    if (!coverage.meetsTarget) recs.push(`Increase test coverage by ${coverage.gap} points to meet the ${coverage.target}% target.`);
    if (!Array.isArray(designDoc.failureModes) || !designDoc.failureModes.length) {
  ...
  buildReviewReport(findings = []) {
    ...
    return {
      status: verdict === 'APPROVE' ? 'PASS' : (verdict === 'REQUEST_CHANGES' ? 'BLOCKED' : 'CONCERNS'),
      findings,
      verdict,
