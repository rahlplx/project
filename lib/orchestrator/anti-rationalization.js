...
const EXCUSE_TABLE = [
  {
    ...
    rebuttal:
      'Prototypes become production code more often than not. The gate cost is the same either way; skipping it now just defers the cost to someone with less context.',
  },
  {
    id: 'add_tests_later',
    ...
    rebuttal:
      'Tests written after the fact verify what the code happens to do, not what it should do. Write the test first while the intended behavior is still explicit.',
  },
  {
    id: 'user_in_a_hurry',
    ...
    rebuttal:
      'A skipped security or correctness check that causes a rollback costs more time than the check itself. Speed comes from not having to redo the work.',
  },
  {
    id: 'looks_fine',
    ...
    rebuttal:
      'Code can execute without errors and still contain a security hole, a logic bug, or an accessibility failure. "Ran without crashing" is not a passing condition.',
  },
  {
    id: 'small_change',
    ...
    rebuttal:
      'Blast radius is not proportional to diff size. A one-line change to an auth check or a shared config can be the highest-risk change in the codebase.',
  },
];
