...
// ⚡ Bolt: Cache enriched context for 30s to avoid redundant I/O on repeated calls
...
class QueryEnricher {
  ...
  enrich(query, options = {}) {
    ...
    if (!this._goalBlockCached) {
      const originalRead = this.ctx.readGoalBlock.bind(this.ctx);
      let cached;
      this.ctx.readGoalBlock = () => {
        if (cached === undefined) cached = originalRead();
        return cached;
      };
      this._goalBlockCached = true;
    }

    // ⚡ Bolt: Return cached result if available for same query + state
