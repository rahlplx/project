...
class QueryEnricher {
  ...
  enrich(query, options = {}) {
    ...
    try {
      ...
      pipelineSpan = getTracer('enricher', this.root).startSpan(
        'enricher.pipeline',
    ...
    try {
      ...
      if (fs.existsSync(kbPath)) {
        ...
        const hits = this._recall(query, kb)
          ...
          .filter(e => e.key);
        if (hits.length) {
          sources.push({ type: SOURCES.KNOWLEDGE_BASE, data: hits });
        }
      }
    } catch {
      /* degrade */
      ...
      if (fs.existsSync(logPath)) {
        ...
        if (latest) {
          ...
          if (
            ...
            (session.recentCommits || session.topFiles)
          ) {
            if (Array.isArray(session.topFiles)) {
              session.topFiles = session.topFiles
                .map(f => this._sanitize(String(f)))
                .filter(Boolean);
            }
    ...
    if (pipelineSpan) {
      try {
        pipelineSpan
          ...
          .setAttribute(
            'template',
            selectedTemplate ? `${selectedTemplate.category}/${selectedTemplate.id}` : 'none'
          )
          .end();
      } catch {
        /* degrade */
  ...
  _sanitize(str) {
    ...
    return str
      ...
      .filter(c => {
        ...
        // Additional BiDi: Arabic LTR mark, isolates, line/para separators
