  ...
  _render(sources, query) {
    ...
    for (const s of sources) {
      if (s.type === SOURCES.SKILL_ROUTE) {
        lines.push(`Skills: ${s.data.skills.join(', ')} — ${s.data.reason}`);
      } else if (s.type === SOURCES.GOAL_BLOCK) {
        lines.push(`Session: ${s.data.goal} (resume: ${s.data.resumeWith || 'continue'})`);
      } else if (s.type === SOURCES.KNOWLEDGE_BASE) {
        lines.push(`Known: ${s.data.map(e => e.key).join(', ')}`);
      } else if (s.type === SOURCES.GIT_CONTEXT && s.data.topFiles) {
        lines.push(`Hot files: ${(s.data.topFiles || []).slice(0, 3).join(', ')}`);
      } else if (s.type === SOURCES.CHAIN_OF_THOUGHT && s.data.reasoning_chain) {
        lines.push(s.data.reasoning_chain.join(' → '));
      }
    }
    return lines.join('\n');
  }
