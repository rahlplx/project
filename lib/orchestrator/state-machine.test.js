...
describe('StateMachine', () => {
  ...
  it('allowBacktrack option permits backward phase transitions', () => {
    // Without flag: forward-skip is blocked
    assert.strictEqual(stateMachine.canTransition('build', 'think'), false);
    // With flag: backtrack is allowed
    assert.strictEqual(stateMachine.canTransition('build', 'think', { allowBacktrack: true }), true);
  });

  it('allowBacktrack does not affect valid forward transitions', () => {
    assert.strictEqual(stateMachine.canTransition('think', 'plan', { allowBacktrack: true }), true);
  });
