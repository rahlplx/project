...
const ROUTING_TABLE = [
  {
    ...
    reason:
      'Auth work carries the highest security surface area — security audit + tests are mandatory.',
  },
  {
    id: 'payment',
  ...
  {
    id: 'deploy-infra',
    patterns: [/\b(deploy|ship|production|staging|hosting|release)\b/i, /\bci[\s/]?cd\b|\bcicd\b|\bci\b.{0,10}\bcd\b/i],
...
class SkillRouter {
  route(request) {
    ...
    const matched = ROUTING_TABLE.filter(entry => entry.patterns.some(p => p.test(text)));
