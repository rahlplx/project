...
describe('QueryEnricher._selectTemplate', () => {
  it('returns full level at confidence >= 0.6', () => {
    const qe = new QueryEnricher();
    const result = qe._selectTemplate(['tdd-vibe'], 0.65);
  ...
  it('returns medium level at confidence 0.35–0.59', () => {
    const qe = new QueryEnricher();
    const result = qe._selectTemplate(['security-audit'], 0.45);
    assert.ok(result);
  ...
  it('returns minimal level at confidence < 0.35', () => {
    const qe = new QueryEnricher();
    const result = qe._selectTemplate(['one-click-vercel'], 0.2);
    assert.ok(result);
  ...
  it('picks first matching skill from array', () => {
    const qe = new QueryEnricher();
    const result = qe._selectTemplate(['unknown-skill', 'code-review'], 0.7);
    assert.ok(result);
    assert.strictEqual(result.category, 'refactor');
    assert.strictEqual(result.skill, 'code-review');
  });
});
...
describe('SKILL_TO_TEMPLATE', () => {
  ...
  it('maps tdd-vibe to testing category', () => {
    assert.strictEqual(SKILL_TO_TEMPLATE['tdd-vibe'], 'testing');
  });

  it('maps security-audit to security category', () => {
    assert.strictEqual(SKILL_TO_TEMPLATE['security-audit'], 'security');
  });

  it('maps planning-agent to ai-agent category', () => {
    assert.strictEqual(SKILL_TO_TEMPLATE['planning-agent'], 'ai-agent');
  });
});
