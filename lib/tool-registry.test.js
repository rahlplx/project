...
describe('ToolRegistry', () => {
  ...
  it('getUnusable returns unusable tools with reasons', () => {
    const registry = new ToolRegistry();
    registry.register('bad', { isUsable: () => false, category: 'deploy', metadata: { name: 'Bad Tool' } });
    registry.register('good', { isUsable: () => true, category: 'deploy', metadata: { name: 'Good Tool' } });
  ...
  it('isUsable that throws causes tool to be treated as unusable', () => {
    const registry = new ToolRegistry();
    registry.register('throws', { isUsable: () => { throw new Error('check failed'); }, category: 'deploy', metadata: {} });
    const usable = registry.findUsable('deploy');
