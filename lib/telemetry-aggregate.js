...
function generateCrossProjectTrends() {
  ...
  const sortedErrors = Object.entries(errorTypes)
    .sort(([, a], [, b]) => b - a);
  const mostCommonError = sortedErrors.length > 0
    ? { type: sortedErrors[0][0], pct: Math.round((sortedErrors[0][1] / sessions.length) * 100) }
    : null;

  // Top failure pattern
  const sortedFailures = Object.entries(harnessFailures)
    .sort(([, a], [, b]) => b - a);
  ...
  return {
    ...
    mostStuckPhase,
    mostCommonError,
    harnessPassRate: harnessCheckTotal > 0
      ? Math.round((harnessPassTotal / harnessCheckTotal) * 100) + '%'
      : null,
