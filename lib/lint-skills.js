...
function lintFile(filePath, providedContent) {
    ...
    if (descMatch && descMatch[1].length > 1024) {
      warnings.push(
        makeIssue(
          filePath,
          'desc-too-long',
          `description is ${descMatch[1].length} chars (max 1024 — truncated in agent system prompts)`
        )
      );
    }
  } else {
  ...
  const skillMdPath = path.join(path.dirname(filePath), 'SKILL.md');
  if (!fs.existsSync(skillMdPath)) {
    errors.push(makeIssue(filePath, 'missing-skill-md', 'SKILL.md is absent — agents cannot discover this skill'));
  }
