...
function scan(options = {}) {
  ...
  const patterns = [
    ...
    { id: 'A03-innerHTML', re: /innerHTML\s*=/, desc: 'innerHTML assignment' },
    {
      ...
      re: /password\s*=\s*['"][^'"]{4,}['"]|secret\s*=\s*['"][^'"]{4,}['"]/,
...
function _loadEvolvedRules() {
  try {
    if (fs.existsSync(EVOLVED_RULES_FILE)) {
      const parsed = JSON.parse(fs.readFileSync(EVOLVED_RULES_FILE, 'utf8'));
  ...
  try {
    ...
    for (const [token, count] of Object.entries(tokenFreq)) {
      if (count >= 3 && !newPatterns.some(p => p.pattern.includes(token))) {
        newPatterns.push({
          id: `evolved-${token.toLowerCase()}`,
          pattern: `\\b${token}\\b`,
          flags: 'i',
          desc: `Evolved: frequent token '${token}'`,
          source: 'auto',
        });
        changed = true;
      }
    ...
    if (changed || !current.last_scan) {
      ...
      fs.writeFileSync(
        EVOLVED_RULES_FILE,
        JSON.stringify(
          { version: 1, patterns: newPatterns, last_scan: new Date().toISOString() },
          null,
          2
        ) + '\n',
        'utf8'
