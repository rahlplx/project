...
const { formatTextReport, formatJsonReport, formatMarkdownReport } = require('./security-scan.report');
...
const DETECTION_PATTERNS = [
  {
    id: 'ASI01-001', category: 'ASI01', severity: CRITICAL, title: 'Instruction Override',
    ...
    remediation: 'Remove instruction override directives; skills should not change agent instructions',
    falsePositiveGuard: /example|demonstrat|educational|sample/i,
  },
  {
    id: 'ASI01-002', category: 'ASI01', severity: CRITICAL, title: 'DAN/Jailbreak',
  ...
  {
    id: 'ASI01-003', category: 'ASI01', severity: CRITICAL, title: 'Identity Override',
    pattern: /you\s+are\s+(?:now\s+)?(?:an?\s+)?(?:unfiltered|free|independent|ungoverned)\s+(?:AI|assistant|entity)/i,
  ...
  {
    id: 'ASI01-004', category: 'ASI01', severity: HIGH, title: 'System Message Impersonation',
  ...
  {
    id: 'ASI01-005', category: 'ASI01', severity: CRITICAL, title: 'Escaped Instruction Override',
    pattern: /`{3,}[\s\S]{0,500}(?:ignore|override)\s+(?:all\s+)?(?:previous|prior)\s+(?:instructions?|commands?)/i,
  ...
  {
    id: 'ASI01-006', category: 'ASI01', severity: HIGH, title: 'Description with Override',
    pattern: /(?:description|purpose).{0,30}(?:override|ignore|bypass)\s+(?:safety|security|restrictions?|filter|limit)/i,
  ...
  {
    id: 'ASI02-001', category: 'ASI02', severity: CRITICAL, title: 'Exfiltration Endpoint',
    pattern: /(?:fetch|axios|request)\s*\(\s*["'`](?:https?:\/\/)[^"'`]*(?:api|collect|log|ingest|exfil|upload|track)/i,
  ...
  {
    id: 'ASI02-002', category: 'ASI02', severity: CRITICAL, title: 'Fetch + Credential Concat',
    pattern: /(?:fetch|axios|request)\s*\(\s*["'`][^"'`]{0,100}["'`]\s*\+\s*[a-zA-Z_$][\w$]*(?:[kK]ey|token|secret|pass|api|cred)/i,
  ...
  {
    id: 'ASI02-003', category: 'ASI02', severity: HIGH, title: 'Network + System Module Combo',
    type: 'pair',
    pairPatterns: [/require\s*\(\s*["'](?:https?|http|net|tls)["']\s*\)/i, /require\s*\(\s*["'](?:fs|child_process)["']\s*\)/i],
  ...
  {
    id: 'ASI02-004', category: 'ASI02', severity: CRITICAL, title: 'Credential File Read',
    pattern: /(?:readFile(?:Sync)?|readdir(?:Sync)?)\s*\(\s*["'`][^"'`]*(?:\.env|credentials|\.aws|\.ssh|secrets?\.[a-z]+|token\.[a-z]+)/i,
  ...
  {
    id: 'ASI02-005', category: 'ASI02', severity: HIGH, title: 'Environment Secret Access',
    pattern: /process\.env\.\s*(?:API[_-]?(?:KEY|TOKEN|SECRET)|TOKEN|SECRET|PASSWORD|CREDENTIAL|PRIVATE[_-]KEY)/i,
    description: 'Skill accesses sensitive environment variables',
    remediation: 'Minimize access to sensitive environment variables; scope to specific credentials',
    falsePositiveGuard: /example|test/i,
  },
  {
    id: 'ASI02-006', category: 'ASI02', severity: HIGH, title: 'Base64 Encode Sensitive Data',
  ...
  {
    id: 'ASI02-007', category: 'ASI02', severity: HIGH, title: 'XHR + Credential URL',
    pattern: /(?:XMLHttpRequest|XDomainRequest|fetch)\s*\([^)]*["'`][^"'`]*(?:api[_-]?key|token|password|secret)/i,
  ...
  {
    id: 'ASI03-001', category: 'ASI03', severity: CRITICAL, title: 'execSync Arbitrary',
    pattern: /(?:execSync|exec\s*\()\s*[^)]*(?:\$|\+|process\.env|input|userData|payload|body|req)/i,
  ...
  {
    id: 'ASI03-002', category: 'ASI03', severity: CRITICAL, title: 'Spawn Shell',
    pattern: /(?:spawn|exec(?:File)?Sync?)\s*\(\s*["'`](?:sh|bash|cmd|powershell|zsh|dash|pwsh)["'`]/i,
  ...
  {
    id: 'ASI03-003', category: 'ASI03', severity: HIGH, title: 'Dangerous rm/dd',
  ...
  {
    id: 'ASI03-004', category: 'ASI03', severity: HIGH, title: 'Unsafe Module Loading',
    pattern: /require\s*\(\s*[^"'`\s)]/i,
    description: 'Skill dynamically requires modules using variables, enabling arbitrary code loading',
    remediation: 'Use static require statements with string literals; avoid variable module paths',
    falsePositiveGuard: /example|test|fixture/i,
  },
  {
    id: 'ASI03-005', category: 'ASI03', severity: HIGH, title: 'Command Injection',
    pattern: /(?:exec(?:Sync|File)?|spawn(?:Sync)?)\s*\([^)]*(?:['"`][^)]*['"`]\s*\+|`[^`]*\${)|shell\s+exec\b|concat\s*\([^)]*\$[a-z_]/i,
  ...
  {
    id: 'ASI03-006', category: 'ASI03', severity: MEDIUM, title: 'chmod 777',
  ...
  {
    id: 'ASI03-007', category: 'ASI03', severity: HIGH, title: 'Arbitrary Sudo',
  ...
  {
    id: 'ASI05-001', category: 'ASI05', severity: HIGH, title: 'JSON.parse User Input',
    pattern: /JSON\.parse\s*\(\s*[^"'`\d][^)]{0,100}(?:request|response|body|input|data|user|external|untrusted)/i,
  ...
  {
    id: 'ASI05-002', category: 'ASI05', severity: CRITICAL, title: 'eval + JSON',
  ...
  {
    id: 'ASI05-003', category: 'ASI05', severity: HIGH, title: 'Unsafe YAML',
  ...
  {
    id: 'ASI05-004', category: 'ASI05', severity: HIGH, title: 'Function Constructor',
    pattern: /new\s+Function\s*\(\s*(?:["'`]|[a-z])[^)]*[+$]/i,
    description: 'Skill creates functions from strings using dynamic input, enabling code injection',
    remediation: 'Avoid dynamic Function constructor; use predefined functions instead',
    falsePositiveGuard: /example|test|fixture/i,
  },
  {
    id: 'ASI05-005', category: 'ASI05', severity: HIGH, title: 'Deserialize/Pickle',
  ...
  {
    id: 'ASI06-001', category: 'ASI06', severity: HIGH, title: 'Context Push with Override',
    pattern: /(?:context|memory|history|chat|messages)\b\.\s*(?:push|add|append|unshift|set)\s*\([^)]*(?:override|system|instruction|ignore)/i,
    description: 'Skill pushes override instructions into agent context/memory',
    remediation: 'Remove context poisoning attempts; do not inject override instructions into memory',
    falsePositiveGuard: /example|educational/i,
  },
  {
    id: 'ASI06-002', category: 'ASI06', severity: HIGH, title: 'Mass Memory Wipe',
    pattern: /(?:context|memory|history|chat|messages)\s*=\s*(?:\[\s*\]|\{\s*\})|\.\s*length\s*=\s*0|clear\s*(?:Context|Memory|History)/i,
  ...
  {
    id: 'ASI06-003', category: 'ASI06', severity: MEDIUM, title: 'Memory Injection',
  ...
  {
    id: 'ASI06-004', category: 'ASI06', severity: MEDIUM, title: 'History Manipulation',
  ...
  {
    id: 'ASI06-005', category: 'ASI06', severity: MEDIUM, title: 'Stored Override Data',
  ...
  {
    id: 'ASI07-001', category: 'ASI07', severity: MEDIUM, title: 'Version 0.x Dependency',
  ...
  {
    id: 'ASI07-002', category: 'ASI07', severity: LOW, title: 'Stale Import',
    pattern: /require\s*\(\s*["'`][^"'`]*(?:deprecated|abandoned|legacy|unmaintained)[^"'`]*["'`]\s*\)/i,
  ...
  {
    id: 'ASI07-003', category: 'ASI07', severity: MEDIUM, title: 'Unresolved Security TODO',
  ...
  {
    id: 'ASI07-004', category: 'ASI07', severity: LOW, title: 'Stale/Deprecated Reference',
  ...
  {
    id: 'ASI08-001', category: 'ASI08', severity: LOW, title: 'Long Base64 String',
  ...
  {
    id: 'ASI08-002', category: 'ASI08', severity: LOW, title: 'Scanner Disable Comment',
    pattern: /(?:scan(?:ner|ning)?\s*(?:bypass|skip|ignore|disable)|(?:bypass|skip|ignore|disable)\s*scan(?:ner|ning)?)/i,
  ...
  {
    id: 'ASI08-003', category: 'ASI08', severity: MEDIUM, title: 'Hex Encoded String',
  ...
  {
    id: 'ASI08-004', category: 'ASI08', severity: LOW, title: 'Character Obfuscation',
  ...
  {
    id: 'ASI08-005', category: 'ASI08', severity: HIGH, title: 'eval + Decode',
  ...
  {
    id: 'ASI08-006', category: 'ASI08', severity: LOW, title: 'Long Regex Test',
...
function classifySeverity(category, pattern, context) {
  const baseMap = {
    ASI01: CRITICAL, ASI02: CRITICAL, ASI03: CRITICAL,
    ASI05: HIGH, ASI06: HIGH, ASI07: MEDIUM, ASI08: MEDIUM,
  };
  const base = pattern && typeof pattern.severity === 'number' ? pattern.severity : (baseMap[category] || INFO);
...
module.exports = { scanSkill, scanAllSkills, classifySeverity, formatReport, checkHarness, severityToString };
