...
const { formatTextReport, formatMarkdownReport } = require('./security-scan.report');
...
const DETECTION_PATTERNS = [
  {
    ...
    cwe: 'CWE-20',
    owasp: 'OWASP-PROMPT-INJECTION-01',
  ...
  },
  {
    ...
    owasp: 'OWASP-INJECTION-03',
    pattern:
      /(?:(?:const|let|var)\s+([A-Za-z_$][\w$]*)\s*=\s*require\s*\(\s*['"`]child_process['"`]\s*\)[\s\S]{0,300}\b\1\.exec\s*\()|(?:require\s*\(\s*['"`]child_process['"`]\s*\)\s*\.exec\s*\()/,
    description:
      'child_process required and .exec() called — injection risk if args are user-controlled',
    remediation: 'Use execFileSync with a fixed command array; avoid exec() with variable args',
    falsePositiveGuard: /\b(?:example|educational|fixture)\b/i,
    falsePositivePathGuard: /(?:^|[/\\])(?:test|example|fixture)s?[/\\]/i,
  },
  {
    id: 'ASI03-002',
    category: 'ASI03',
  ...
  },
  {
    ...
    owasp: 'OWASP-INJECTION-03',
    pattern:
      /(?:exec(?:Sync|File)?|spawn(?:Sync)?)\s*\([^'"`+)]{0,100}(?:['"`][^'"`+)]{0,100}['"`]\s{0,5}\+|\+\s{0,5}['"`]|`[^`]{0,200}\${)|shell\s+exec\b|concat\s*\([^)]{0,100}\$[a-z_]/i,
  ...
  },
  {
    ...
    cwe: 'CWE-502',
    owasp: 'OWASP-DESERIALIZATION-05',
    pattern:
...
function scanSkill(filePath, content) {
  ...
  for (const dp of DETECTION_PATTERNS) {
    ...
    if (dp.falsePositivePathGuard && dp.falsePositivePathGuard.test(filePath)) continue;
    if (dp.falsePositiveGuard) {
      // Test the lines containing the match, not the full file — avoids suppression from
      // unrelated "test" words elsewhere, while still catching same-line/same-block context.
      // For pair-type detectors matchStr is synthetic ("p1...p2"), so search for the first part.
      const searchStr = matchStr.includes('...') ? matchStr.split('...')[0] : matchStr;
      const anchor = searchStr.substring(0, Math.min(searchStr.length, 100));
      const matchPos = content.indexOf(anchor);
      const matchEnd = matchPos >= 0 ? matchPos + matchStr.length : 0;
      const lineBegin = matchPos >= 0 ? content.lastIndexOf('\n', matchPos) + 1 : 0;
      const rawLineEnd = matchPos >= 0 ? content.indexOf('\n', matchEnd) : -1;
      const lineEnd = rawLineEnd === -1 ? content.length : rawLineEnd;
      const matchLine = content.substring(lineBegin, lineEnd);
      if (dp.falsePositiveGuard.test(matchLine)) continue;
    }
    ...
    findings.push({
      ...
      severity: classifySeverity(dp.category, dp, null),
      detector: 'regex',
      title: dp.title,
