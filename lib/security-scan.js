...
function scanSkill(filePath, content) {
  ...
  for (const dp of DETECTION_PATTERNS) {
    ...
    if (dp.falsePositiveGuard) {
      // Test the line containing the match, not the full file — avoids suppression from
      // unrelated "test" words elsewhere, while still catching same-line context like "// example".
      const matchPos = content.indexOf(matchStr.substring(0, Math.min(matchStr.length, 30)));
