...
describe('buildIndex', () => {
  ...
  test('category is derived from directory structure', () => {
    const result = buildIndex({ rootDir: projectRoot });
    for (const skill of result.skills) {
      expect(skill.category).toMatch(/^(deploy|deployment|design|explain|knowledge|orchestration|preview|progress|quality|setup|testing-qa|workflow)$/);
    }
  });
...
describe('readIndex / writeIndex', () => {
  ...
  test('writeIndex with single arg builds index from rootDir (regression)', () => {
    // Previously writeIndex(root) with no index arg wrote undefined to disk → 0 entries
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'index-test-'));
    try {
      // Seed a minimal skill so buildIndex finds something
