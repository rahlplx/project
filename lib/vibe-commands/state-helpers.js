...
function validatePhase(commandPhase, state, options = {}) {
  ...
  if (allowBacktrack && cmdIdx < currentIdx) {
    return { valid: true, message: `↺ Backtracking to '${commandPhase}' from '${current}' (--backtrack)` };
  }
...
function writeHandoff(from, to, phase, context) {
  ...
  content = content
    ...
    .replace(/Phase \[N\] -- \[think\/plan\/break\/build\/harness\/review\/ship\/retro\/learn\/evolve\/done\]/g, `Phase -- ${phase}`)
    ...
    .replace(/\[What has been completed so far -- be specific\]/g, context.state || 'Phase in progress')
    ...
    .replace(/\[Specific, measurable deliverable description\]/g, context.need || `${phase} phase completion`)
    .replace(/\[ \] \[Criterion 1 -- measurable\]/g, context.acceptance ? context.acceptance.map(c => `- [ ] ${c}`).join('\n') : '- [ ] Phase artifacts exist')
    .replace(/\[Links to specs, designs, previous work\]/g, context.refs || 'Internal pipeline')
    .replace(/\[Specific quality criteria for this deliverable\]/g, context.quality || 'All checks pass')
    .replace(/\[What proof of completion looks like\]/g, context.evidence || 'Artifacts in state.json')
