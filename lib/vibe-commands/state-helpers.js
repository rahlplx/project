...
function writeJSON(p, d) {
  // TODO(Bug #4): This writeJSON function uses a non-atomic read-modify-write
  // pattern. When two processes write to the same file concurrently (e.g.,
  // CLI + background auto-maintain both updating lifecycle.json), the second
  // write can clobber the first, causing counter resets (e.g., interaction_count
  // resetting to 2 instead of 0 after maintenance).
  //
  // Fix: Use temp-file + rename (atomic on POSIX) instead of direct writeFileSync:
  //   const tmp = p + '.tmp.' + process.pid;
  //   fs.writeFileSync(tmp, JSON.stringify(d, null, 2) + '\n', 'utf8');
  //   fs.renameSync(tmp, p);
  //
  // Deferred because:
  // 1. The race is low-impact (counter resets don't cause data loss, just
  //    inaccurate maintenance triggering)
  // 2. Hard to test deterministically (requires concurrent process spawning)
  // 3. M1 (PR #30) fixed the higher-impact version of this race in otel-tracer.js
  //
  // Refs: vibe-stack /vibe-review consensus, M5 milestone
  // See: lib/telemetry/otel-tracer.js (M1 fix for the same pattern)
...
function recordTelemetry(commandName) {
  // ⚡ Bolt: Consolidated telemetry + lifecycle update into a single write cycle
  // Reduces I/O overhead and avoids redundant file handle creation.
  try {
    const { recordCommand } = require(TELEMETRY_TRACKER);
    recordCommand(commandName);
  } catch (e) {
    /* telemetry tracker optional */
  }

  try {
    const lc = readJSON(LIFECYCLE_PATH) || {};
    lc.interaction_count = (lc.interaction_count || 0) + 1;
    lc.total_interactions = (lc.total_interactions || 0) + 1;
    lc.updated = new Date().toISOString();

    writeJSON(LIFECYCLE_PATH, lc);
    return lc; // Return updated lifecycle for immediate use (prevents reload)
  } catch {
    return null;
  }
}

// Single source of truth — imported from state-machine to eliminate divergence
const { PHASE_ORDER } = require('../orchestrator/state-machine');
