  ...
  try {
    ...
    if (fs.existsSync(telemetryPath)) {
      ...
      if (sessions.length > 0) {
        try {
          const latest = JSON.parse(fs.readFileSync(path.join(telemetryPath, sessions[sessions.length - 1]), 'utf8'));
          if (latest.meta) telemetry = { ...telemetry, ...latest.meta, total_sessions: sessions.length };
        } catch { /* degrade */ }
      }
    }
    ...
    if (retro.telemetry) {
      ...
      const sessionLabel = sessionCount > 0 ? sessionCount : '0 (run auto-maintain to generate session data)';
