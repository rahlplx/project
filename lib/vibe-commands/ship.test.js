...
describe('ship handler', () => {
  it('returns ok status on dry run (no --push)', () => {
    ...
    assert.ok(['ok', 'error', 'tests-failed', 'security-blocked', 'supply-chain-blocked'].includes(result.status));
  });
  ...
  it('returns trust-gate when --push used without confirm at low trust level', () => {
    ...
    assert.ok(['trust-gate', 'ok', 'error', 'tests-failed', 'security-blocked', 'supply-chain-blocked', 'env-not-ignored'].includes(result.status));
  });
});
