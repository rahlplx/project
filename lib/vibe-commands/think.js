...
const handler = (args, state) => {
  ...
  if (grillResult.needsGrilling) {
    try { recordClarification(); } catch { /* telemetry optional */ }
    console.log(
  ...
  if (tmpl) {
    try {
      ...
      const loaded = templateId ? new PromptTemplates().getTemplate(tmpl.category, templateId) : null;
      if (loaded) {
        console.log(`  \u2500\u2500\u2500 Suggested Template: ${loaded.name} (${tmpl.category}/${templateId}) \u2500\u2500\u2500`);
        console.log(`  ${loaded.prompt.split('\n').slice(0, 4).join('\n  ')}`);
        console.log();
      }
    } catch { /* prompt-templates optional */ }
  }
