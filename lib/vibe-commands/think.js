...
const handler = (args, state) => {
  ...
  if (grillResult.needsGrilling) {
    console.log('  \x1b[33m\u26a1 GRILL GATE \u2014 Request needs clarification before planning\x1b[0m\n');
    ...
    console.log('  \x1b[2mExample: npx vibe-stack think "build a SaaS tool for X that does Y"\x1b[0m\n');
  }
  ...
  if (routeResult.skills[0] !== 'vibe') {
    console.log(`  \x1b[1mRecommended skills:\x1b[0m ${routeResult.skills.map(s => `/${s}`).join(', ')}`);
