...
const handler = (args, state) => {
  ...
  const bandLabel = (dial, val) => {
    const b = ts.bandFor(dial, val);
    return b ? b.label : '—';
  };

  console.log('  \x1b[1mTaste-Skill Dials:\x1b[0m');
  console.log(
    `  DESIGN_VARIANCE:  ${dials.DESIGN_VARIANCE}  (${bandLabel('DESIGN_VARIANCE', dials.DESIGN_VARIANCE)})`
  );
  console.log(
    `  MOTION_INTENSITY: ${dials.MOTION_INTENSITY}  (${bandLabel('MOTION_INTENSITY', dials.MOTION_INTENSITY)})`
  );
  console.log(
    `  VISUAL_DENSITY:   ${dials.VISUAL_DENSITY}  (${bandLabel('VISUAL_DENSITY', dials.VISUAL_DENSITY)})`
  );
  if (usedBaseline) {
    console.log('  \x1b[2m(baseline — no vibe-word signal detected in brief)\x1b[0m');
  }
