...
const handler = (args, state) => {
  ...
  let framework = 'unknown', language = 'unknown', testFramework = 'unknown', buildCommand = null;
  ...
  if (fs.existsSync(pkgPath)) {
    ...
    if (pkg.scripts?.dev) framework = pkg.scripts.dev.includes('next') ? 'next' : pkg.scripts.dev.includes('vite') ? 'vite' : 'node';
  }
  ...
  const stack = { framework, language, testFramework, testCommand: `npx ${testFramework}`, buildCommand, lintCommand: null };
