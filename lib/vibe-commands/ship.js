...
const handler = (args, state) => {
  ...
  try {
    ...
    const status = execFileSync('git', ['status', '--porcelain'], { cwd: PROJECT_ROOT, timeout: 10000, encoding: 'utf8' }).trim();
    if (status) {
    ...
    try {
      execFileSync('npm', ['test'], { cwd: PROJECT_ROOT, timeout: 120000, encoding: 'utf8', stdio: 'pipe' });
    ...
    try {
      const auditJson = execFileSync('npm', ['audit', '--omit=dev', '--json'], { cwd: PROJECT_ROOT, timeout: 30000, encoding: 'utf8', stdio: 'pipe' });
      const audit = JSON.parse(auditJson);
      const vulns = audit.metadata?.vulnerabilities || {};
      const critical = vulns.critical || 0;
      const high = vulns.high || 0;
      if (critical > 0) {
        console.log(`  \x1b[31m✖ SUPPLY CHAIN: ${critical} critical vulnerabilities — run npm audit fix\x1b[0m`);
        return { status: 'supply-chain-blocked', critical, high };
      } else if (high > 0) {
        console.log(`  \x1b[33m⚠  Supply chain: ${high} high vulnerabilities — review before shipping\x1b[0m`);
      } else {
        console.log(`  Supply chain clean (${(vulns.moderate || 0) + (vulns.low || 0)} low/moderate)`);
      }
    } catch (e) {
    ...
    const branch = execFileSync('git', ['rev-parse', '--abbrev-ref', 'HEAD'], { cwd: PROJECT_ROOT, timeout: 5000, encoding: 'utf8' }).trim();
    ...
    for (const file of scannedFiles.slice(0, 15)) {
      ...
      if (!result.clean) {
        complexityFindings.push(...result.findings.map(f => ({ file: path.relative(PROJECT_ROOT, file), ...f })));
      }
    }
    if (complexityFindings.length) {
      console.log(`  \x1b[33m⚠  Complexity: ${complexityFindings.length} function(s) above threshold\x1b[0m`);
      complexityFindings.slice(0, 3).forEach(f => console.log(`    ${f.file}:${f.line} ${f.name}() — score ${f.score} (${f.message})`));
    } else {
    ...
    if (args.includes('--push') || args.includes('-p')) {
      if (pushRequiresConfirm && !args.includes('--confirm-push')) {
        ...
        console.log('  Re-run with --push --confirm-push to proceed, or set state.trustLevel >= 3 to skip this gate.\n');
      ...
      execFileSync('git', ['commit', '-m', 'chore: ship via vibe-stack pipeline'], { cwd: PROJECT_ROOT, timeout: 10000 });
      const upstream = execFileSync('git', ['rev-parse', '--abbrev-ref', 'HEAD@{upstream}'], { cwd: PROJECT_ROOT, timeout: 5000, encoding: 'utf8' }).trim();
