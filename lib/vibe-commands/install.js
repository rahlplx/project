...
const IDE_FLAGS = { '--cursor': 'cursor', '--windsurf': 'windsurf', '--claude-code': 'claude-code' };
...
const handler = (args = [], state) => {
  ...
  if (args.includes('--all')) {
    ...
    if (!detected) {
      console.log('  [install] Could not auto-detect an IDE. Pass --cursor, --windsurf, --claude-code, or --all.');
  ...
  for (const name of targets) {
    ...
    if (skillResult) {
      console.log(`  [install:${name}] Skills synced: ${skillResult.written.length}, skipped: ${skillResult.skipped.length}`);
    }
  }
