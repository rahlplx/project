...
const handler = (args, state) => {
  ...
  if (subcommand === 'capture') {
    try {
      ...
      execFileSync(process.execPath, [learnCapture, ...args.slice(1)], {
        cwd: PROJECT_ROOT,
        timeout: 30000,
        encoding: 'utf8',
        stdio: 'inherit',
  ...
  if (!gl.error) {
    ...
    if (types.length) {
      console.log(`  Commit types: ${types.map(([t, n]) => `${t}(${n})`).join(', ')}`);
    }
    if (gl.topChangedFiles.length) {
      console.log(
        `  Most-changed files: ${gl.topChangedFiles
          .slice(0, 3)
          .map(f => f.file)
          .join(', ')}`
      );
    ...
    if (gl.velocity) {
      console.log(`  Velocity: ${gl.velocity.commitsLast7Days} commits in last 7 days`);
    }
