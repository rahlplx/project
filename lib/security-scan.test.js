...
describe('severityToString', () => {
  test('returns CRITICAL for 10', () => {
    assert.strictEqual(severityToString(10), 'CRITICAL');
  });
  test('returns HIGH for 7', () => {
    assert.strictEqual(severityToString(7), 'HIGH');
  });
  test('returns MEDIUM for 4', () => {
    assert.strictEqual(severityToString(4), 'MEDIUM');
  });
  test('returns LOW for 1', () => {
    assert.strictEqual(severityToString(1), 'LOW');
  });
  test('returns INFO for 0', () => {
    assert.strictEqual(severityToString(0), 'INFO');
  });
  test('returns UNKNOWN for unknown', () => {
    assert.strictEqual(severityToString(99), 'UNKNOWN');
  });
});
...
describe('ASI01 - Malicious Skills', () => {
  ...
  test('non-match on clean skill content', () => {
    const clean =
      'class Safe { constructor() { this.name = "x"; this.description = "docs"; } } module.exports = Safe;';
...
describe('ASI02 - Supply Chain', () => {
  ...
  test('non-match on clean skill content', () => {
    const clean =
      'class Safe { constructor() { this.name = "x"; } run() { return "ok"; } } module.exports = Safe;';
    const result = scanSkill('safe.js', clean);
...
describe('ASI03 - Over-Privileged', () => {
  ...
  test('detects command injection in template literal (ASI03-005)', () => {
    const result = scanSkill('test.js', 'exec(`rm -rf ${dir}`)');
    assert.ok(result.findings.some(f => f.id === 'ASI03-005'));
  });

  test('does not flag benign template literal (ASI03-005)', () => {
    const result = scanSkill('test.js', 'const x = `hello ${name}` ');
    assert.strictEqual(result.findings.filter(f => f.id === 'ASI03-005').length, 0);
  });
...
describe('ASI06 - Memory Poisoning', () => {
  test('detects context push with override (ASI06-001)', () => {
    const result = scanSkill(
      'test.js',
      "context.push({ role: 'system', content: overrideInstruction })"
    );
  ...
  test('does not flag benign context usage', () => {
    const clean =
      'const ctx = { messages: [] }; ctx.messages.push("hello"); module.exports = { ctx };';
    const result = scanSkill('safe.js', clean);
...
describe('formatReport', () => {
  ...
  test('json output is valid JSON', () => {
    ...
    assert.strictEqual(parsed.totalScanned, 50);
...
describe('scanAllSkills integration', () => {
  test('discovers all skills', () => {
    ...
    assert.ok(result.totalScanned >= 49);
  });
  ...
  test('no CRITICAL findings in existing skills', () => {
    ...
    assert.strictEqual(
      crits.length,
      0,
      `Found ${crits.length} critical: ${JSON.stringify(crits.slice(0, 3))}`
    );
  });
...
describe('falsePositiveGuard — tests match snippet not full file', () => {
  test('flags ASI03-001 even when file contains benign word "test" elsewhere', () => {
    // Bug: guard was testing full file content, so any file with "test" anywhere suppressed ASI03 findings
    const content = `
      // This is a test helper utility
      function runCommand() {
        execSync('rm ' + input);
      }
    `;
    const result = scanSkill('script.js', content);
    assert.ok(
      result.findings.some(f => f.id === 'ASI03-001'),
      'Should flag ASI03-001 even when benign "test" word appears in the same file'
    );
  });

  test('does not flag ASI03-001 when same line has trailing "// example" comment', () => {
    // The guard checks the full line containing the match, so "example" in a same-line comment suppresses
    const content = "execSync('rm ' + input) // example usage documented in SKILL.md";
    const result = scanSkill('script.js', content);
    assert.ok(
      !result.findings.some(f => f.id === 'ASI03-001'),
      'Should not flag when "example" appears on the same line as the match'
    );
  });
});

describe('ASI03-001b - Dynamic require exec', () => {
  test('detects require child_process + .exec() call', () => {
    const code = "const cp = require('child_process');\ncp.exec(cmd);";
    const result = scanSkill('test.js', code);
    assert.ok(result.findings.some(f => f.id === 'ASI03-001b'));
  });

  test('detects inline require(...).exec()', () => {
    const code = "require('child_process').exec(userCmd);";
    const result = scanSkill('test.js', code);
    assert.ok(result.findings.some(f => f.id === 'ASI03-001b'));
  });

  test('does not flag require child_process + execFileSync (safe pattern)', () => {
    const code =
