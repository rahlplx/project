...
function formatTextReport(summary) {
  ...
  for (const [cat, catData] of Object.entries(summary.categories)) {
    ...
    lines.push(sub);
    const catFindings = catData.findings.length > 0 ? catData.findings : summary.findings.filter(f => f.category === cat);
  ...
  lines.push(sep);
  lines.push(`  ${overallPass ? GREEN : RED}${BOLD}Overall: ${overallPass ? '✓ PASS' : '✗ FAIL'}${RESET}  ${overallPass ? '(0 critical)' : `(${summary.criticalCount} critical)`}`);
  lines.push(sep);
